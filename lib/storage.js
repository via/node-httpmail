...
function claimUuid(mailbox, callback) {
...
function storeMessage(mailbox, tags, data, score, id, callback) {
  ...
  client.multi()
    .sadd([mailbox, 'folders'].join('.'), tags)
    .hset([mailbox, 'messages'].join('.'), id, tag)
    .zadd([mailbox, tags].join('/'), score, id)
...
Storage.prototype.newMessage = function (mailbox, tags, data, score, callback) {
  var self = this;

  claimUuid.call(this, mailbox, function (id) {
    return storeMessage.call(self, mailbox, tags, data, score, id, callback);
  });
};
...
Storage.prototype.getList = function (mailbox, filters, sorts, start, stop, callback) {
  var self = this;
  var client = this.newRedisClient();
  if (start === undefined) {
    start = 0;
  ...
  client.zrevrange([mailbox, 'messages'].join('/'), start, stop, function (err, res) {
    client.quit();
    assert.ifError(err);
    process.nextTick(function () {
      callback(res);
...
Storage.prototype.newTag = function(mailbox, tag, callback) {
  var client = this.newRedisClient();
  client.sadd([mailbox, 'folders'].join('.'), tag, function (err, res) {
    client.quit();
    assert.ifError(err);
    process.nextTick(function () {
      callback(res);
...
Storage.prototype.getTags = function (mailbox, callback) {
  var client = this.newRedisClient();
...
Storage.prototype.getTotalCounts = function (mailbox, tags, callback) {
  var client = this.newRedisClient();
  var ret = {};
  var numFolders = tags.length;
  var seenFolders = 0;

  function buildCallback(tag) {
  ...
  for (i=0; i<numFolders; i++) {
    var key = [mailbox, tags[i]].join('/');
    client.zcard(key, buildCallback(tags[i]));
  }
};
...
Storage.prototype.getUnreadCount = function (mailbox, tag, callback) {
  var client = this.newRedisClient();
